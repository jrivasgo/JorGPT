...
def test_sha256_file(tmp_path):
    ...
    assert (
        result
        == "02208429d06f4ddbfa652244e493c452395e3cda72adc127bec779e2b7911b47"
    )


def test_write_tokenizer_sample_creates_sample_and_manifest(
    tmp_path,
    monkeypatch,
):
    texts = [
        "Café",
        "España",
        "Este documento no debería llegar a escribirse.",
    ]
    ...
    monkeypatch.setattr(
        ...
        fake_iter_tokenizer_texts,
    )

    output_path = tmp_path / "sample.txt"

    manifest = tokenizer_sample.write_tokenizer_sample(
        output_path,
        target_chars=12,
    )
    ...
    assert (
        ...
        == tokenizer_sample.sha256_file(output_path)
    )
    ...
    assert (
        ...
        == tokenizer_sample.DATASET_REVISION
    )
...
def test_write_tokenizer_sample_rejects_invalid_target(tmp_path):
    ...
    with pytest.raises(ValueError):
        tokenizer_sample.write_tokenizer_sample(
            output_path,
            target_chars=0,
        )


def test_write_tokenizer_sample_rejects_negative_progress_interval(tmp_path):
    output_path = tmp_path / "sample.txt"
...
def test_write_tokenizer_sample_detects_exhausted_dataset(
    tmp_path,
    monkeypatch,
):
    def fake_iter_tokenizer_texts(seed, shuffle_buffer, min_chars):
